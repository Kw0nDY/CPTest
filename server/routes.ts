...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // 설비 유지보수 데이터 초기화 (CSV 가져오기)
  app.post('/api/maintenance/import', async (req, res) => {
    try {
      await maintenanceService.importCSVData();
      res.json({ success: true, message: '설비 유지보수 데이터를 성공적으로 가져왔습니다.' });
    } catch (error) {
      console.error('CSV 데이터 가져오기 실패:', error);
      res.status(500).json({ error: 'CSV 데이터 가져오기에 실패했습니다.' });
    }
  });
  ...
  app.post('/api/chat/conversations/:conversationId/messages', async (req, res) => {
    try {
      ...
      const response = await chatService.processMaintenanceQuery(conversationId, message);
      res.json({ response });
    } catch (error) {
  ...
  // 설비 지식 검색
  app.post('/api/maintenance/search', async (req, res) => {
    try {
      ...
      const results = await maintenanceService.searchMaintenanceKnowledge(query);
      res.json({ results });
    } catch (error) {
      console.error('설비 지식 검색 실패:', error);
      res.status(500).json({ error: '설비 지식 검색에 실패했습니다.' });
    }
  });
