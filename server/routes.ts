      ...
      if (config) {
        try {
          if (hasUserData) {
            ...
            // 🔥 Flowise API 호출 (새로운 깨끗한 벡터 DB 사용)
            console.log(`🔥 Flowise API 호출 시작 (새로운 깨끗한 인스턴스):`, {
              메시지: message,
              세션: sessionId
            });
            
            // ✅ 순수 질문만 전송 (업로드된 파일은 이미 벡터 DB에 저장됨)
            ...
            if (flowiseResponse.success) {
              ...
              console.error('❌ Flowise API 실패:', flowiseResponse);
              aiResponse = '업로드하신 데이터를 기반으로 답변할 수 없습니다. 잠시 후 다시 시도해주세요.';
