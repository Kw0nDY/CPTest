...
export async function registerRoutes(app: any) {
  ...
  app.get(
    "/api/chatbot-data-integrations/:configId",
  ...
  app.delete(
    "/api/chatbot-data-integrations/:configId/:dataSourceId",
    async (req: Request, res: Response) => {
      try {
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const config = configId
          ? await storage.getChatConfiguration(configId)
          : null;
        ...
        if (config?.uploadedFiles) {
          console.log(
            `🔍 업로드된 파일 확인: ${config.uploadedFiles.length}개`,
          );
            ...
            if (file.type === "source_code" && file.content) {
              ...
              if (file.content.includes("vector/upsert")) {
                ...
                if (match) {
                  ...
                  console.log(
                    `✅ 소스 파일에서 API URL 추출: ${extractedApiUrl}`,
                  );
                }
              }
            }
            ...
            if (file.type === "csv" && file.content) {
              ...
              try {
                ...
                if (lines.length > 1) {
                  ...
                  console.log(
                    `✅ CSV 데이터 로드: ${file.name} → ${dataRows.length}개 레코드`,
                  );
                }
              } catch (csvError) {
        ...
        if (!isDirectSourceApiCall && config?.chatflowId) {
          ...
          isDirectSourceApiCall = true;
          console.log(
            `✅ config에서 vector/upsert API 구성: ${config.chatflowId} → ${extractedApiUrl}`,
          );
        }

        console.log(
          `⚡ 간단한 API 호출 모드 - 업로드된 데이터: ${allUploadedData.length}개 레코드`,
        );
        ...
        if (config) {
          try {
            ...
            if (isDirectSourceApiCall && extractedApiUrl) {
              // 업로드 데이터가 없다면 upsert 호출 안 함
              if (allUploadedData.length === 0) {
                aiResponse = `질문을 받았습니다: "${message}"\n\n현재 업로드된 데이터가 없어 벡터 업설트는 생략했습니다.`;
              } else {
                // ✅ Node 18+ WHATWG FormData 사용 (form-data 패키지 X)
                const formData = new FormData();

                // CSV 만들기
                const firstItem = allUploadedData[0];
                const columns = Object.keys(firstItem); // ← 실제 헤더
                // 가장 적절한 텍스트 컬럼을 고르거나, 특정 규칙으로 선택
                const textColumn = columns.includes("text")
                  ? "text"
                  : columns[0]; // 예: 우선 text, 없으면 첫 컬럼

                const csvHeader = columns.join(",");
                const csvRows = allUploadedData.map((item) =>
                  columns
                    .map((col) => {
                      const val = (item[col] ?? "").toString();
                      // 간단한 CSV escape
                      const needQuote = /[",\n]/.test(val);
                ...
                // WHATWG FormData에 파일 첨부
                const blob = new Blob([csvContent], { type: "text/csv" });
                formData.append("files", blob, "uploaded_data.csv");

                // 폼 필드
                formData.append("columnName", textColumn); // ✅ 실제 존재하는 헤더명 사용
                formData.append(
                  "metadata",
                  JSON.stringify({
                ...
                const response = await fetch(extractedApiUrl, {
                  method: "POST",
                  body: formData, // WHATWG FormData는 헤더 자동 세팅
                });
                ...
                aiResponse =
                  ...
                  apiResult.message ||
                  apiResult.result ||
                  `데이터 업로드 성공. ${allUploadedData.length}개 레코드를 벡터 DB에 저장했습니다.`;
              }
            } else {
